...
$result=$objVenta->consultarVentaPorDetalle('consultar',$f,$f);
$result2=$objVenta->consultarVentaPorDetalle('EFECTIVO',$f,$f);
$result3=$objVenta->consultarVentaPorDetalle('TARJETA',$f,$f);
...
if(isset($_POST['buscar_dato'])){
    ...
    $result2=$objVenta->consultarVentaPorDetalle('EFECTIVO',$fechaini,$fechafin);
    $result3=$objVenta->consultarVentaPorDetalle('TARJETA',$fechaini,$fechafin);
...
while($row2=mysql_fetch_array($result2)){
       ...
       $subt = $row2['Total2'];
       $tot_efectivo=$tot_efectivo+$row2['Total'];
...
while($row3=mysql_fetch_array($result3)){
    ...
    $tot_tarjeta=$tot_tarjeta+$row3['Total'];
    
}
$totalTarjetaVentas = $tot_tarjeta-($tot_tarjeta-$subt2);
$totalEfectivoVentas = $tot_efectivo-($tot_efectivo-$subt);
$ganancia = $tot_ganancia - (($tot_tarjeta-$subt2)+($tot_efectivo-$subt));
$totalgeneral = $totalEfectivoVentas + $totalTarjetaVentas;
...
<table width="100%" border="0">
    ...
    <tr>
        ...
        <td align="center"><input name="txtEfectivo" type="text" disabled id="textfield" class="input_text" value="<?php echo "$ ".number_format($totalEfectivoVentas, 2, '.', ''); ?>" size="10" style="background-color:#FFC"></td>
        <td align="center"><input name="txtTarjeta" type="text" disabled id="textfield" class="input_text" value="<?php echo "$ ".number_format($totalTarjetaVentas, 2, '.', ''); ?>" size="10" style="background-color:#FFC"></td>
        <td align="center"><input name="txtTotal" type="text" disabled id="textfield" class="input_text" value="<?php echo "$ ".number_format($totalgeneral, 2, '.', ''); ?>" size="10" style="background-color:#FFC"></td>
        <td align="center">
              <input name="txtGanancia" type="text" disabled id="textfield4" class="input_text" value="<?php echo "$ ".number_format($ganancia, 2, '.', ''); ?>" size="10" style="background-color: #DFF; ">
        </td>
    </tr>
</table>
