...
function enviarPDF()
{
	...
	window.open("reporte_promocion.php?busqueda="+busqueda,'win2','status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=640,height=480,directories=no,location=no');
return false;
...
{
	var busqueda=document.datos.busqueda.value;
	window.open("imprimir_promocion.php?busqueda="+busqueda,'win2','status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=640,height=480,directories=no,location=no');
return false;	
}
</script>
</head>
...
$result_tot=$objPromocion->consultarTotalPromocions();
...
$result=$objPromocion->consultarPromocionPorParametro($criterio,$busqueda,$pages->limit);

if(isset($_POST['buscar_dato'])){
			
	$busqueda=$_POST["txtDato"];
	$result=$objPromocion->consultarPromocionPorParametro('descripcion',$busqueda,$pages->limit);
...
while($row=mysql_fetch_array($result)){
	...
	echo "<td width='1%' align='center'>".$row['IdPromocion']."</td>";
	echo "<td width='5%' align='center'><a href=detalle_promocion.php?idpromocion=".$row["IdPromocion"]." title='Ver Detalle'><img src='../img/Iconfinder/search-16.png'></td>";
    echo "<td width='5%' align='center'><a href=editar_promocion.php?idpromocion=".$row["IdPromocion"]." title='Editar'><img src='../img/Iconfinder/page_edit.png'></td>";
	echo "</tr>";
}
