...
function enviarPDF()
{
	...
	window.open("reporte_talla.php?busqueda="+busqueda,'win2','status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=640,height=480,directories=no,location=no');
return false;
...
{
	var busqueda=document.datos.busqueda.value;
	window.open("imprimir_talla.php?busqueda="+busqueda,'win2','status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=640,height=480,directories=no,location=no');
return false;	
}
</script>
</head>
...
$result_tot=$objtalla->consultarTotalTallas();
...
$result=$objtalla->consultarTallaPorParametro($criterio,$busqueda,$pages->limit);

if(isset($_POST['buscar_dato'])){
			
	$busqueda=$_POST["txtDato"];
	$result=$objtalla->consultarTallaPorParametro('descripcion',$busqueda,$pages->limit);
...
while($row=mysql_fetch_array($result)){
	...
	echo "<td width='1%' align='center'>".$row['IdTalla']."</td>";
	echo "<td width='5%' align='center'><a href=detalle_talla.php?idtalla=".$row["IdTalla"]." title='Ver Detalle'><img src='../img/Iconfinder/search-16.png'></td>";
    echo "<td width='5%' align='center'><a href=editar_talla.php?idtalla=".$row["IdTalla"]." title='Editar'><img src='../img/Iconfinder/page_edit.png'></td>";
	echo "</tr>";
}
