...
function guardarFactura($productos,$idtipodoc,$idclie,$idemp,$serie,$numfact,$valorventa,$igv,$total,$estado,$pago,$promo,$descuento,$referencia,$ttotal){
	$con = new clsConexion;
	if($con->conectarse()==true){
		$query="CALL SP_I_Venta($idtipodoc,$idclie,$idemp,'$serie','$numfact',now(),$valorventa,$igv,$total,'$estado','$pago','$promo','$descuento','$referencia','$ttotal')";
		$result = @mysql_query($query);				 
		if(!$result){
...
function cancelarventa($id,$estado){
   $con = new clsConexion;
   if($con->conectarse()==true){
     $query = "CALL SP_U_ActualizarVentaEstado('$id','$estado')";
	 $result = @mysql_query($query);
	 if (!$result){
	 	echo "Error en la Transaccion: ".mysql_error();
	 }
