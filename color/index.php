...
function enviarPDF()
{
	...
	window.open("reporte_color.php?busqueda="+busqueda,'win2','status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=640,height=480,directories=no,location=no');
return false;
...
{
	var busqueda=document.datos.busqueda.value;
	window.open("imprimir_color.php?busqueda="+busqueda,'win2','status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=640,height=480,directories=no,location=no');
return false;	
}
</script>
</head>
...
<div class="block">
    ...
    <div class="block_head"> 
    	...
    	<div class="titulo_head">Gestor de Colores</div>
...
<button type="submit" class="button" onClick="enviarPDF();">
                   <span style="background-image:url(../img/Iconfinder/PDF2.png);" title="Convertir a PDF">
                    ...
                    <td>
                        <a href="registrar_color.php" class="toolbar">
...
$result_tot=$objcolor->consultarTotalColors();
...
$result=$objcolor->consultarColorPorParametro($criterio,$busqueda,$pages->limit);

if(isset($_POST['buscar_dato'])){
			
	$busqueda=$_POST["txtDato"];
	$result=$objcolor->consultarColorPorParametro('descripcion',$busqueda,$pages->limit);
...
while($row=mysql_fetch_array($result)){
	...
	echo "<td width='1%' align='center'>".$row['IdColor']."</td>";
	echo "<td width='5%' align='center'><a href=detalle_color.php?idcolor=".$row["IdColor"]." title='Ver Detalle'><img src='../img/Iconfinder/search-16.png'></td>";
    echo "<td width='5%' align='center'><a href=editar_color.php?idcolor=".$row["IdColor"]." title='Editar'><img src='../img/Iconfinder/page_edit.png'></td>";
	echo "</tr>";
}
